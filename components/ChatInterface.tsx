...
interface Props {
  ...
  onAcceptAllContacts: (persons: Person[]) => void;
          ...
          <div key={gIdx} className="space-y-6">
            ...
            {group.messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col group ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                {msg.contactProposals && (
                    <div className="mt-4">
                      {msg.contactProposals.length > 1 && (
                        <button
                          onClick={() => onAcceptAllContacts(msg.contactProposals!)}
                          className="w-full mb-3 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-all shadow-md flex items-center justify-center gap-2"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                          Add All {msg.contactProposals.length} Contacts
                        </button>
                      )}
                      {msg.contactProposals.map((p) => (
                          <div className="mb-2" key={`contact-${p.name}`}><ContactProposalView person={p} onAccept={() => onAcceptContact(p)} onReject={() => onRejectContact(p)} /></div>
                      ))}
                    </div>
                )}
              </div>
